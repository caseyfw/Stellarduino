...
void PrintMoonIlluminationAndPhase(double JD) 
{
...
void PrintSunAndMoonInfo(int year, int month, int day, double longitude, double latitude) 
{
  CAADate CalcDate(year, month, day, true);
  double JD = CalcDate.Julian();
  printf("Times are in UTC\n");
  CAARiseTransitSetDetails sun_rise_transit_set(GetSunRiseTransitSet(JD, longitude, latitude));
  printf("Sun:\n");
...
int main()
{
  ...
  //Array for city name
  const char* city_names[] = {"Buenos Aires", "Moscow", "Dublin", "Tampa", "Cape Town", "North Pole"};
    
  //Array for longitudes / latitudes {longitude, latitude}
  //east longitude and south latitude are negative values
  double longitudes_latitudes[6][2] = {{58.3817, -34.6036}, {-37.6178, 55.7517}, {6.2661, 53.3428}, {82.4586, 27.9472}, {-18.4244, -33.9767}, {0.0, 90.0} };
    
  //Array for time offsets {time zone. daylight savings}
  int time_offsets[6][2] = {{-3, 0}, {4, 0}, {0, 1}, {-5, 1}, {2, 0}, {0, 0}};
    
  //Print out possible bug for moon overhead transit times for East latitudes
  for (int i=0; i<=5; ++i)
  {
    ...
    PrintSunAndMoonInfo2(city_names[i], year, month, day, time_offset, longitudes_latitudes[i][0], longitudes_latitudes[i][1]);
  }
  ...
  {
    day = i;
    PrintSunAndMoonInfo(year, month, day, longitude, latitude);
  }


  /*
  //Code to write out all the JD of year values from 2012 to 2021
  long year = 2012;
  bool bContinue = true;
  while (bContinue)
  {
  ...
  */
  ...
  while (bContinue2)
  {
    double JDUTC = (CAADynamicalTime::DeltaT(JDUT1) - CAADynamicalTime::CumulativeLeapSeconds(JDUT1) - 32.184) / 86400 + JDUT1;
    printf("%f\t%f\n", JDUT1, (JDUT1 - JDUTC) * 86400);
  ...
  */
  ...
  while (bContinue3)
  {
    ...
    printf("%f\t%f\t%f\t%f\t%f\n", JDD, DeltaT, LeapSeconds, TTC, TTC - DeltaT);
