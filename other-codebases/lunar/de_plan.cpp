...
void * DLL_FUNC load_ps1996_series( FILE *ifile, double jd, int planet_no)
{
   ...
   if( !fread( p.fqs, header.total_fqs, sizeof( double), ifile))
      read_error = -3;
   if( !fread( block_sizes, header.n_blocks, sizeof( int16_t), ifile))
      read_error = -4;
   for( i = 0; i < block; i++)
      jump += (long) block_sizes[i];
   fseek( ifile, jump, SEEK_CUR);
   tbuff = (char *)malloc( block_sizes[block]);
   if( tbuff && !fread( tbuff, block_sizes[block], 1, ifile))
      read_error = -5;
   if( !tbuff || read_error)
      {
      free( rval);
